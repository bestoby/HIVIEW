...
ext_data_type_t g_mipi_ext_data_type_os08a20_12bit_8m_nowdr_attr = {
    .devno = 0,
    .num = MIPI_NUM,
...
combo_dev_attr_t g_mipi_4lane_chn0_sensor_imx485_12bit_8m_nowdr_attr = {
    .devno = 0,
    .input_mode = INPUT_MODE_MIPI,
    .data_rate = MIPI_DATA_RATE_X1,
    .img_rect = {0, 0, WIDTH_3840, HEIGHT_2160},
...
combo_dev_attr_t g_mipi_4lane_chn2_sensor_imx485_12bit_8m_nowdr_attr = {
    .devno = 2, /* dev 2 */
    .input_mode = INPUT_MODE_MIPI,
    .data_rate = MIPI_DATA_RATE_X1,
    .img_rect = {0, 0, WIDTH_3840, HEIGHT_2160},
...
combo_dev_attr_t g_mipi_4lane_chn0_sensor_imx334_12bit_8m_nowdr_attr = {
    .devno = 0,
    .input_mode = INPUT_MODE_MIPI,
    .data_rate = MIPI_DATA_RATE_X1,
    .img_rect = {0, 0, WIDTH_3840, HEIGHT_2160},
...
combo_dev_attr_t g_mipi_4lane_chn2_sensor_imx334_12bit_8m_nowdr_attr = {
    .devno = 2, /* dev 2 */
    .input_mode = INPUT_MODE_MIPI,
    .data_rate = MIPI_DATA_RATE_X1,
    .img_rect = {0, 0, WIDTH_3840, HEIGHT_2160},
...
static hi_void sample_comm_vi_get_mipi_attr(sample_sns_type sns_type, combo_dev_attr_t *combo_attr)
{
    ...
    switch (sns_type) {
        ...
        case SONY_IMX334_MIPI_8M_60FPS_12BIT:
                ...
                (hi_void)memcpy_s(combo_attr, sizeof(combo_dev_attr_t),
                    &g_mipi_4lane_chn0_sensor_imx334_12bit_8m_nowdr_attr, sizeof(combo_dev_attr_t));
	...
	mppex_comm_vi_get_mipi_attr(sns_type, combo_attr, &ob_height);
...
static hi_void sample_comm_vi_get_mipi_ext_data_attr(sample_sns_type sns_type, ext_data_type_t *ext_data_attr)
{
	...
	mppex_comm_vi_get_mipi_ext_data_attr(sns_type, ext_data_attr);
}
                                                      ...
                                                      combo_dev_attr_t *combo_attr)
{
    ...
    switch (sns_type) {
        ...
        case SONY_IMX485_MIPI_8M_30FPS_12BIT:
            ...
            if (vi_dev == 0) {
                ...
                (hi_void)memcpy_s(combo_attr, sizeof(combo_dev_attr_t),
                    &g_mipi_4lane_chn2_sensor_imx334_12bit_8m_nowdr_attr, sizeof(combo_dev_attr_t));
            }
            combo_attr->mipi_attr.input_data_type = combo_attr->mipi_attr.input_data_type;
            break;
	...
	mppex_comm_vi_get_mipi_attr_by_dev_id(sns_type, vi_dev, combo_attr, &ob_height);
    combo_attr->img_rect.height = combo_attr->img_rect.height + ob_height;
}
...
hi_void sample_comm_vi_get_dev_attr_by_intf_mode(hi_vi_intf_mode intf_mode, hi_vi_dev_attr *dev_attr)
{
    switch (intf_mode) {
	...
	mppex_comm_vi_get_dev_attr_by_intf_mode(intf_mode, dev_attr);
}
...
hi_void sample_comm_vi_get_size_by_sns_type(sample_sns_type sns_type, hi_size *size)
{
	...
	mppex_comm_vi_get_size_by_sns_type(sns_type, size);
}
...
hi_u32 sample_comm_vi_get_obheight_by_sns_type(sample_sns_type sns_type)
{
	...
	mppex_comm_vi_get_obheight_by_sns_type(sns_type, &ob_height);
    return ob_height;
}
...
static hi_u32 sample_comm_vi_get_pipe_num_by_sns_type(sample_sns_type sns_type)
{
    switch (sns_type) {
        ...
        default:
		{
			int n = mppex_comm_vi_get_pipe_num_by_sns_type(sns_type);
			return (n>0)?n:1;
		}
    }
}
...
hi_void sample_comm_vi_get_default_mipi_info(sample_sns_type sns_type, sample_mipi_info *mipi_info)
{
    ...
    mipi_info->divide_mode = LANE_DIVIDE_MODE_1;//maohw LANE_DIVIDE_MODE_0;
...
hi_void sample_comm_vi_get_default_dev_info(sample_sns_type sns_type, sample_vi_dev_info *dev_info)
{
	...
	mppex_comm_vi_get_default_dev_info(sns_type, dev_info);
                                             ...
                                             sample_vi_pipe_info pipe_info[])
{
    ...
    for (i = 0; i < bind_pipe->pipe_num; i++) {
		...
		mppex_comm_vi_get_default_pipe_info(sns_type, i, pipe_info, &size);
    }
}
...
static hi_void sample_comm_vi_deregister_sensor_lib(hi_vi_pipe vi_pipe)
{
...
static hi_s32 sample_comm_vi_start_one_pipe_isp(hi_vi_pipe vi_pipe, hi_u8 pipe_index, const sample_vi_cfg *vi_cfg)
{
    ...
    printf("%s => vi_pipe:%d, pipe_index:%d, vi_cfg:%p\n", __func__, vi_pipe, pipe_index, vi_cfg);
    ...
    printf("%s => vi_pipe:%d, sample_comm_vi_register_sensor_lib()\n", __func__, vi_pipe);
    ...
    printf("%s => vi_pipe:%d, hi_mpi_isp_mem_init()\n", __func__, vi_pipe);
    ...
    printf("%s => vi_pipe:%d, hi_mpi_isp_set_pub_attr()\n", __func__, vi_pipe);
    ...
    printf("%s => vi_pipe:%d, hi_mpi_isp_init()\n", __func__, vi_pipe);
    ...
    printf("%s => vi_pipe:%d, sample_comm_isp_sensor_founction_cfg()\n", __func__, vi_pipe);
...
static hi_void sample_comm_vi_stop_one_pipe_isp(hi_vi_pipe vi_pipe)
{
    hi_mpi_isp_exit(vi_pipe);
...
hi_s32 sample_comm_vi_start_vi(const sample_vi_cfg *vi_cfg)
{
    ...
    hi_vi_dev vi_dev;
    //maohw
    if(vi_cfg->mipi_info.mipi_dev < 0)
    {
		sample_print("bypass mipi_rx, sns_type:%d\n", vi_cfg->sns_info.sns_type);
	}
	else
	{
    ...
    //maohw bypass isp;
	if(mppex_comm_vi_bypass_isp(vi_cfg))
	{
		sample_print("bypass isp, sns_type:%d\n", vi_cfg->sns_info.sns_type);
		return HI_SUCCESS;
	}
                                              ...
                                              sample_vi_user_frame_info *user_frame_info)
{
    ...
    if (vb_blk == HI_VB_INVALID_HANDLE) {
        sample_print("hi_mpi_vb_get_blk err, size:%d\n", calc_cfg->vb_size);
        return HI_FAILURE;
    }
...
{
    (hi_void)snprintf_s(file_name, length, length - 1, "FPN_frame_%dx%d_%dbit.raw", //maohw
                        video_frame->width, video_frame->height, hi_vi_get_raw_bit_width(video_frame->pixel_format));
}
...
{
    hi_s32 err;
    err = snprintf_s(file_name, length, length - 1, "%s/FPN_frame_%dx%d_%dbit_iso%d.raw", //maohw
                     dir_name, video_frame->width, video_frame->height,
                     hi_vi_get_raw_bit_width(video_frame->pixel_format), iso);
    if (err < 0) {
        return HI_FAILURE;
    }
    return HI_SUCCESS;
...
static hi_void *sample_common_vi_send_pipe_frame_proc(hi_void *param)
{
    ...
    const hi_u32 milli_sec = -1; /* milli_sec: -1 */
...
static hi_s32 sample_comm_vi_read_user_frame_file(hi_vi_pipe vi_pipe, sample_vi_user_frame_info *user_frame_info)
{
    hi_s32 ret;
    FILE *pfd = HI_NULL;
    const hi_char *frame_file = "./UsePic_3840x2160_sp420.yuv";
    hi_size frame_size = {WIDTH_3840, HEIGHT_2160};
...
static hi_bool sample_vi_is_frame_pts_suitable(hi_u64 pts1, hi_u64 pts2)
{
    hi_s64 pts_diff;
...
static hi_s32 sample_vi_match_wdr_pts(hi_video_frame_info frame_info[], hi_vi_pipe vi_pipe[], hi_s32 pipe_num)
{
    hi_s32 cnt = 0;
    hi_s32 try_time = 5; /* try 5 times */
    const hi_s32 millsec = 1000; /* millsec: 1000ms */
    hi_s32 ret;
    ...
    while (cnt++ < try_time) {
        ...
        if (sample_vi_is_frame_pts_suitable(pts_max, pts_min) == HI_TRUE) {
            return HI_SUCCESS;
        }
...
static hi_s32 sample_vi_send_pipe_wdr_frame(hi_vi_pipe send_pipe, hi_video_frame_info frame_info[],
                                            hi_s32 pipe_num, hi_s32 millsec)
{
    const hi_video_frame_info *wdr_frame_info[HI_VI_MAX_WDR_FRAME_NUM] = {HI_NULL};
    hi_s32 i, ret;

    for (i = 0; i < pipe_num; i++) {
        wdr_frame_info[i] = &frame_info[i];
    }
    ret = hi_mpi_isp_run_once(send_pipe);
    if (ret != HI_SUCCESS) {
        printf("pipe[%d] isp runonce failed\n", send_pipe);
        return ret;
    }
    ret = hi_mpi_vi_send_pipe_raw(send_pipe, wdr_frame_info, pipe_num, millsec);
    if (ret != HI_SUCCESS) {
        printf("pipe[%d] send frame failed\n", send_pipe);
        return ret;
    }
    ret = hi_mpi_isp_get_vd_time_out(send_pipe, HI_ISP_VD_BE_END, millsec);
    if (ret != HI_SUCCESS) {
...
static hi_void *sample_vi_wdr_dump_and_send_proc(hi_void *param)
{
    hi_vi_frame_dump_attr dump_attr = {
        .depth = 2,
        .enable = HI_TRUE
    };
    const hi_s32 millsec = 1000;
    hi_video_frame_info dump_frame_info[HI_VI_MAX_WDR_FRAME_NUM];
    hi_vi_bind_pipe *bind_pipe = (hi_vi_bind_pipe*)param;
    hi_vi_pipe *vi_pipe = bind_pipe->pipe_id;
    hi_s32 pipe_num = bind_pipe->pipe_num;
    hi_vi_pipe send_pipe = vi_pipe[0];
    hi_s32 i, ret;
    ...
    ret = hi_mpi_vi_set_pipe_frame_source(send_pipe, HI_VI_PIPE_FRAME_SOURCE_USER);
    if (ret != HI_SUCCESS) {
    ...
    while (g_send_pipe_pthread) {
        for (i = 0; i < pipe_num; i++) {
            ret = hi_mpi_vi_get_pipe_frame(vi_pipe[i], &dump_frame_info[i], millsec);
            if (ret != HI_SUCCESS) {
        ...
        if (sample_vi_match_wdr_pts(dump_frame_info, vi_pipe, pipe_num) != HI_SUCCESS) {
...
release:
        for (i = i - 1; i >= 0; i--) {
            (hi_void)hi_mpi_vi_release_pipe_frame(vi_pipe[i], &dump_frame_info[i]);
        }
    }
...
hi_s32 sample_comm_vi_send_wdr_frame(hi_vi_bind_pipe *bind_pipe)
{
    hi_s32 ret;
    pthread_t thread_id = 0;
    g_send_pipe_pthread = HI_TRUE;
    ret = pthread_create(&thread_id, HI_NULL, sample_vi_wdr_dump_and_send_proc, (hi_void *)bind_pipe);
    if (ret != HI_SUCCESS) {
        printf("vi create send frame thread failed!\n");
        g_send_pipe_pthread = HI_FALSE;
        return HI_FAILURE;
