...
hi_s32 sample_isp_ir_auto_run(hi_vi_pipe vi_pipe)
{
    ...
    ir_attr.en                = g_ast_ir_attr[vi_pipe].en;
    ...
    while (HI_TRUE == g_ast_ir_thread[vi_pipe].thread_flag) 
    {
        ...
        {
          ret = hi_mpi_isp_ir_auto_run_once(vi_pipe, &ir_attr);
          if (HI_SUCCESS != ret) {
...
hi_s32 isp_ir_switch_to_auto(hi_vi_pipe vi_pipe)
{
  ...
  g_vi_pipe = vi_pipe;
  #define RATIO 6.0 //8.0
    
  /* need to modified when sensor/lens/IR-cut/infrared light changed */
  g_ast_ir_attr[g_vi_pipe].en = HI_TRUE;
  g_ast_ir_attr[g_vi_pipe].normal_to_ir_iso_threshold= 400*1000; // normal_to_ir_iso_threshold 16000
  g_ast_ir_attr[g_vi_pipe].ir_to_normal_iso_threshold = 200*1000; // ir_to_normal_iso_threshold 400
